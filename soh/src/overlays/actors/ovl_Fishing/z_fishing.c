...
f32 Fishing_GetMinimumRequiredScore() {
    ...
    if(sLinkAge == 1) {
        weight = CVarGetInteger("gChildMinimumWeightFish", 10);
    } else {
        weight = CVarGetInteger("gAdultMinimumWeightFish", 13);     
    }
...
bool getInstantFish() {
    return CVarGetInteger("gInstantFishing", 0);
}

bool getGuaranteeBite() {
    return CVarGetInteger("gGuaranteeFishingBite", 0);
}

bool getFishNeverEscape() {
    return CVarGetInteger("gFishNeverEscape", 0);
}
